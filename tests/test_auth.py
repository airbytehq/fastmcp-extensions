...
def test_assemble_auth_branches(
    ...
    result = _assemble_auth(
        server=server,
        verifiers=verifiers,  # type: ignore  # The test passes a deliberately invalid verifier collection.
        base_url=None,
    ...
    if expected == "server":
        ...
        assert isinstance(result, expected)  # type: ignore  # The parametrized test supplies runtime type objects.
...
def _oidc_config(**overrides: object) -> OIDCAuthConfig:
    ...
    return OIDCAuthConfig(**kwargs)  # type: ignore  # The test builds configuration from intentionally broad values.
...
def test_oidc_auth_config_is_keyword_only() -> None:
    ...
    with pytest.raises(TypeError):
        OIDCAuthConfig(  # type: ignore  # The test verifies rejection of an invalid configuration.
